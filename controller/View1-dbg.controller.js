	...
	return Controller.extend("com.qb2i4.qb2i4.controller.View1", {
		...
		onInit: function () {
			...
			x1.read(y, {
				...
				success: function (oData, response) {
					...
					for (var a = 0; a < i; a++) {
						//	var us_p = data[a].Plant.slice(0, 2);
						//	if (us_p === "US") {
						g = a;
						p1.pList.push({
							Plant: data[a].Plant
		...
		abcd: function () {
			for (var t = 0; t < p1.pList.length; t++) {
				var us_p = p1.pList[t].Plant.slice(0, 2);
		//		if (us_p === "US") {
...
//}
					...
					for (var a = 0; a < i; a++) {
						...
						if (a !== 0) {
							...
							r.catList.push({
								...
								SalesDocument: data[a].SalesDocument,
								PurchasingGroup: data[a].PurchasingGroup
				...
				for (var a = 0; a < r.catList.length; a++) {
					...
					if (r.catList[a].Plant === plant) {
						n.newList.push({
							...
							SalesDocument: r.catList[a].SalesDocument,
							PurchasingGroup: r.catList[a].PurchasingGroup
						});
					}
				}
					...
					for (var s = 0; s < z.zList.length; s++) {
						...
						if (n.newList[t].PurchaseRequisition === z.zList[s].SourceMRPElement && z.zList[s].MRPElementCategoryName ===
							...
							final.finalList.push({
								...
								SalesDocument: n.newList[t].SalesDocument,
								PurchasingGroup: n.newList[t].PurchasingGroup,
								DeliveryDate: date3,
		...
		onOpen: function () {
			...
			var url = oResourceBundle.getText("https://my301271.s4hana.ondemand.com/ui#PurchaseRequisition-manage?source=lpd").toString().trim();
